...
def decompress_packages(dataset_path: str) -> str:
    """Decompress packages.
    ...
    for counter, file_name in enumerate(dataset_names):
        ...
        try:
            ...
            if file_name.endswith('.tar.gz'):
                ...
                temp_package_path = f'{temp_dataset_path}'
...
def extract_cli():
    ...
    dataset_path = os.path.abspath(decompress_packages(dataset_path))
...
def train_cli():
    ...
    malicous_csv_dir_path = os.path.join(SETTINGS['path']['features'], malicious_dataset_name)
    benign_csv_dir_path = os.path.join(SETTINGS['path']['features'], benign_dataset_name)
    hyperparameters = {}
    ...
    train(malicous_csv_dir_path, benign_csv_dir_path, preprocess, model, action, hyperparameters)
...
if __name__ == '__main__':
    ...
    DATASET_NAMES = os.listdir(settings['path']['datasets'])
    FEATURE_NAMES = os.listdir(settings['path']['features'])
    ...
    parser_train.add_argument('-m', '--malicious', type=str, required=True, help='malicious dataset name', choices=FEATURE_NAMES)
    parser_train.add_argument('-b', '--benign', type=str, required=True, help='benign dataset name', choices=FEATURE_NAMES)
